...
export const useStaticPageData = () => {
  ...
  const updatePage = useCallback(
    async (
      ...
      try {
        const updatedPage = await adminService.updateStaticPage(
          pageId,
          updates,
        );
        setPages((prev) =>
  ...
  const pageTypeExists = useCallback(
    (type: "home" | "about" | "team") => {
...
export const useStaticPageForm = () => {
  ...
  const loadDefaultHomeContent = useCallback(() => {
    const newFormData = {
      title: "Home",
      content: JSON.parse(JSON.stringify(DEFAULT_HOME_PAGE)),
    };
    setFormData(newFormData);
    setOriginalFormData(JSON.parse(JSON.stringify(newFormData)));
  }, []);
  ...
  const getChangedFields = useCallback(() => {
    ...
    const originalContentStr = JSON.stringify(
      sortObjectKeys(originalFormData.content),
    );
